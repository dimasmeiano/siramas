    ...
    <div class="app-content">
        ...
        <div class="row g-4">
            ...
            <div class="card card-primary card-outline mb-4">
                ...
                @endif
                <!--begin::Form-->
                <form class="needs-validation" novalidate method="POST" action="{{ route('notulen.update', $notulen->id) }}" enctype="multipart/form-data">
                    @csrf
                    @method('PUT')
                <!--begin::Body-->
                <div class="card-body">
                    <div class="row g-3">
                        <div class="col-md">
                            ...
                            <select class="form-select" id="kegiatan_id" name="kegiatan_id" required>
                                ...
                                @foreach ($kegiatans as $kat)
                                    <option value="{{ $kat->id }}" {{ $notulen->kegiatan_id == $kat->id ? 'selected' : '' }}>{{ $kat->nama }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="col-md">
                            ...
                            <select class="form-select" id="pemimpin_id" name="pemimpin_id" required>
                                <option value="" disabled>Pilih Pemimpin Rapat</option>
                                @foreach ($members as $item)
                                    <option value="{{ $item->id }}" {{ $notulen->pemimpin_id == $item->id ? 'selected' : '' }}>
                                        {{ $item->nama_lengkap }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="col-md">
                            ...
                            name="isi_notulen"
                            required
                            >{{ old('isi_notulen', $notulen->isi_notulen) }}</textarea>
                            <div class="invalid-feedback">Mohon diisi dengan benar !!!</div>
                        </div>
                        <span></span>
                    </div>
